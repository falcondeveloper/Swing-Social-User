...
export default function EditProfile() {
        ...
        if (userId) {
            ...
            try {
                ...
                if (!advertiserData) {
                    ...
                    console.log(advertiserData?.SwingStyleTags, "=========advertiser data");
    ...
    const handleChangeSwingStyle = (event: React.ChangeEvent<HTMLInputElement>, option: string) => {
        if (event.target.checked) {
            setSelectedSwingStyleOptions((prev) => [...prev, option]);
        } else {
            setSelectedSwingStyleOptions((prev) => prev.filter((item) => item !== option));
        }
    };


    const [profileFiles, setProfileFiles] = useState<File[]>([]); // State for storing profile image files
    const [privateFiles, setPrivateFiles] = useState<File[]>([]); // State for storing private image files
    ...
    const handlePrivateImageChange = (index: number, event: React.ChangeEvent<HTMLInputElement>) => {
        const file = event.target.files?.[0];
        if (file) {
            // Update the preview URLs
    ...
    const handleUploadPrivateImages = async () => {
        if (privateFiles?.length > 0) {
            for (const image of privateFiles) {
                if (image) {
                    try {
                        const imageUrl = await uploadImage(image); // Wait for the image to upload
                        await uploadPrivateImage(imageUrl); // Process the uploaded image URL
                    } catch (error) {
    ...
    const handleSubmitAvatarAndBanner = async (type: string) => {
        //Update Avatar and Banner
        if (croppedAvatar) {
            let avatarURL: any = await uploadImageBase64(croppedAvatar);
            setAvatar(avatarURL);
        }
        if (croppedBanner) {
            let bannerURL: any = await uploadImageBase64(croppedBanner);
            setBanner(bannerURL)
            return bannerURL;
        }
    }
    return (
        <Box sx={{
            ...
            <Box
                ...
                onClick={() => {
                    const fileInput = document.getElementById("banner-upload") as HTMLInputElement | null;
                    if (fileInput) {
                        fileInput.click();
                    }
                }}
            ...
            <CardContent>
                <Box>
                    ...
                    <Box sx={{ display: "flex", gap: 2, alignItems: "center", mt: 3, background: "#272525", }}>
                        {/* Month Picker */}
                        <FormControl size="small" sx={{ minWidth: 100, borderBottom: "2px solid #e91e63" }}>
                            <Select
                                value={month}
                        ...
                        {/* Day Picker */}
                        <FormControl size="small" sx={{ minWidth: 80, borderBottom: "2px solid #e91e63" }}>
                            <Select
                                value={day}
                ...
                <Typography sx={{ color: "white", marginTop: '20px' }}>Give members a quick snippet of why you're here or what's going on with you right now</Typography>
                <TextField
                    value={tagline || ''}
                ...
                <Typography sx={{ color: "white", marginTop: '20px' }}>Tell others what you're all about</Typography>
                        ...
                        <Box>
                            ...
                            >
                                ...
                                >
                                    ...
                                    <MenuItem value="A little extra padding">A little extra padding</MenuItem>
                                </Select>
                            </FormControl>
                            ...
                            <Divider sx={{ mb: 2 }} />
                            <FormControl fullWidth variant="outlined" sx={{ marginTop: "15px", background: "#272525" }}>
                                ...
                                {privateImages[index] ? (
                                    ...
                                    >
                                        ...
                                        <input
                                            ...
                                            type="file"
                                            accept="image/*"
                                            onChange={(event) => handlePrivateImageChange(index, event)}
                                            style={{ display: "none" }}
                                        />
                                    </label>
                                )}
