...
`;
    ...
    const fetchCurrentProfileInfo = async (currentProfileId: any) => {
        if (currentProfileId) {
            try {
                const response = await fetch(`/api/user/sweeping/user?id=${currentProfileId}`);
                if (!response.ok) {
    ...
    const fetchCurrentLoginProfileId = async (currentLoginProfileId: string) => {
        if (currentLoginProfileId) {
            setLoading(true);
            try {
                const response = await fetch(`/api/user/sweeping/user?id=${currentLoginProfileId}`);
                if (!response.ok) {
                    console.error('Failed to fetch advertiser data:', response.statusText);
                    setCustomProfile(undefined);
                    throw new Error(`HTTP error! status: ${response.status}`);
        ...
        try {
            ...
            if(idParam != null){
                const checkResponse = await fetch('/api/user/sweeping/relation', {
                    ...
                    body: JSON.stringify({ pid: profileId, targetid: idParam, newcategory: category }), // Pass the username to check
                });
        ...
        <>
            {isMobile ? (<MobileSweaping />) : (
                ...
                >
                    ...
                    <Box flex={1} display="flex" sx={{ pb: 3, paddingTop: "64px", backgroundColor: '#121212' }}>
                    ...
                    >
                                    ...
                                    >
                                        ...
                                        >
                                            <Card sx={{
                                                ...
                                                <CardContent>
                                                    <Typography variant="h6">{selectedUserProfile?.Username}</Typography>
                                                    <Typography sx={{ fontSize: '1.0rem', fontWeight: 'bold', marginTop: '1px', marginBottom: '5px' }} color="#C2185B">
                                                        {selectedUserProfile?.Location?.replace(", USA", "")}
                                                    </Typography>
                                                    <Typography
                                                        variant="body2"
                                                        color="secondary"
                                                        dangerouslySetInnerHTML={{ __html: selectedUserProfile?.Tagline }}
                                                        sx={{
                        ...
                        <Box
                            sx={{
                                ...
                                gap: { xs: '1rem', sm: '2rem' },
                                zIndex: 1000,
                                '@media (max-height: 700px)': {  // Add responsive styling for small heights
                                    bottom: '10px',
                                    '& .MuiButton-root': {  // Target all buttons
                                        width: '60px',
                                        height: '60px',
                                        '& img': {  // Target the images inside buttons
                                            width: '30px',
                                            height: '30px',
                                        },
                                    },
