...
export default function MobileAttendeeSwing() {
    ...
    const fetchAllUserProfiles = async (userid: any, targetId: any, eventId: any) => {
        try {
            const response = await fetch("/api/user/sweeping/attendee", {
                ...
                body: JSON.stringify({loginid: userid, targetid: targetId, eventid: eventId})
            });
    ...
    const getUserList = async (profileId: string) => {
        try {
            ...
            const response = await fetch("/api/user/sweeping/swipes?id=" + profileId, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                },
        ...
        <>
            ...
            >
                ...
                {idParam !== null ? (
                    ...
                    >
                        <Box position="relative" width="100%" sx={{ height: { lg: 450, md: 380, sm: 380, xs: 'calc(100vh - 210px)', mb: 15 } }}>
                            <Avatar
                        ...
                        <CardContent>
                            <Typography variant="h6" component="div" gutterBottom>
                                ...
                                {selectedUserProfile?.PartnerDateOfBirth && (
                                    <>
                                        {" | "}
                                        {new Date().getFullYear() - new Date(selectedUserProfile.PartnerDateOfBirth).getFullYear()}
            ...
            {memberalarm && parseInt(memberalarm) > 1 ? null : <InstructionModal />}
